...
class SuccessPaymentModal extends StatelessWidget {
  ...
  String _formatTime(DateTime? time) {
    ...
    return 'Hari ini, ${two(t.hour)}:${two(t.minute)} WIB';
  }
            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Text('Lunas • ID: $orderNumber', style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: AppColors.primary)),
                ),
                ...
                Text(
                  state.isOffline
                      ? 'Pesanan #$orderNumber dicatat lokal — server tidak merespons, data akan disinkronkan.'
                      : 'Pesanan #$orderNumber telah diteruskan ke dapur roll dan tercatat pada cloud POS.',
                ...
                NeomorphicContainer(
                  ...
                  child: Column(
                    children: [
                      _receiptRow('Metode Bayar:', state.paymentLabel),
                      const SizedBox(height: 4),
                      _receiptRow('Waktu:', _formatTime(order?.createdAt)),
                      const SizedBox(height: 4),
                      _receiptRow('Total Dibayar:', 'Rp ${formatPrice(order?.total ?? totalBill)}', isBold: true),
                      if (state.selectedMethod == 'cash' && (order?.change ?? state.change) > 0) ...[
                        const SizedBox(height: 4),
                        _receiptRow('Kembalian:', 'Rp ${formatPrice(order?.change ?? state.change)}'),
                      ],
                    ],
                  ),
                ),
